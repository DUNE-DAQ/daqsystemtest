...
if [[ "$DBT_AREA_ROOT" != "" ]] && [[ "`echo $DBT_AREA_ROOT | grep '^/cvmfs'`" == "" ]]; then
    ...
    # add in the paths of the C++ repositories
    ...
    # add in the paths of the Python repositories
...
if [[ $# -eq 0 ]] || [[ "$1" != "local" ]]; then
    ...
    # add in the paths of the Python repositories
    base_rel_py_repos=(`ls -1d ${base_rel_dir}/sourcecode/*/src/*/integtest/*_test.py 2>/dev/null`)
    all_integtest_paths+=("${base_rel_py_repos[@]}")
fi
